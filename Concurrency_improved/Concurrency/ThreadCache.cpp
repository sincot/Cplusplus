...
void* ThreadCache::FetchFromCentralCache(size_t index, size_t size)
{
	...
	// 添加调试信息
	if (actualNum == 0) {
		printf("ERROR: actualNum == 0, index=%zu, size=%zu, batchNum=%zu, MaxSize=%zu\n",
			index, size, batchNum, _freeLists[index].MaxSize());
		exit(1);
	}
...
void ThreadCache::Deallocate(void* ptr, size_t size)	// 释放内存对象
{
	...
	// 随着桶位置的自由链表越来越长，内存块不需要全部留在 Thread Cache，如果桶上的链表太长了，可以做些处理
	if (_freeLists[index].Size() >= _freeLists[index].MaxSize())	// 如果现在桶上链表的长度超过一次批量申请的内存空间，就可以开始释放一段给 central cache
	{
		// 回收给 CentralCache
...
// 回收 Central Cache 内存对象
void ThreadCache::ListToLong(FreeList& list, size_t size)
{
	// 不会回收当前链表所有的内存块，只是取一批内存出来
	void* start = nullptr, * end = nullptr;
	list.PopRange(start, end, list.MaxSize());		// 取一批申请的最大值

	// 将取出的一段链表，归还给 Central Cache
